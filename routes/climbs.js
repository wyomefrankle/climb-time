...
router.post("/:user_id", function(req, res, next) {
  ...
  const {date, grade, location, feels_like, comment, style, tries, lat, lng } = req.body;
  const { user_id } = req.params; // Extract user ID from the URL params

  // Check if required fields are filled in
  if (!date || !grade || !location || !style || !user_id || !lat || !lng) {
  ...
  db(
    `INSERT INTO climbs (date, grade, location, feels_like, comment, style, tries, user_id, lat, lng) VALUES ("${isoDate}", "${grade}", "${location}", "${feels_like}", "${comment}", "${style}", ${tries}, "${user_id}", ${lat}, ${lng})`
  )
